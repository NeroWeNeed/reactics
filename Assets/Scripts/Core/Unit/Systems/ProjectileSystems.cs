...
namespace Reactics.Core.Unit {
    ...
    public class DeleteProjectilesSystem : SystemBase {
        ...
        protected override void OnUpdate() {
            ...
            Entities.ForEach((int entityInQueryIndex, Entity entity, in Reactics.Core.Effects.Projectile proj) =>
            {
    ...
    public class SetTargetPosition : SystemBase {
        protected override void OnUpdate() {
            ...
            Entities.WithNone<MapBody>().ForEach((ref Reactics.Core.Effects.Projectile proj) =>
            {
                if (proj.effect.homing && translationDataFromEntity.HasComponent(proj.targetUnit)) {
    ...
    public class RotateTowardTargetSystem : SystemBase {
        protected override void OnUpdate() {
            Entities.ForEach((ref Rotation rot, in Translation trans, in Reactics.Core.Effects.Projectile proj) =>
            {
                float3 direction = proj.targetPosition - trans.Value;
                rot.Value = quaternion.LookRotation(math.normalize(direction), Vector3.up);
            }).WithChangeFilter<Reactics.Core.Effects.Projectile>().Schedule();
        }
    }
    ...
    public class ArcForwardSystem : SystemBase {
        protected override void OnUpdate() {
            ...
            Entities.WithNone<MapBody>().ForEach((ref Translation trans, ref Reactics.Core.Effects.Projectile proj, in Rotation rot) =>
            {
    ...
    public class DetectCollisionSystem : SystemBase {
        protected override void OnUpdate() {
            ...
            BufferFromEntity<Reactics.Core.Effects.EffectBuffer> effectBufferFromEntity = GetBufferFromEntity<Reactics.Core.Effects.EffectBuffer>(false);
            ...
            Entities.WithNone<MapBody>().ForEach((ref Translation trans, ref Reactics.Core.Effects.Projectile proj, in MapElement mapElement) =>
            {
                ...
                if (!currentPoint.ComparePoints(proj.originPoint) && proj.currentPierce < proj.effect.maxPierce) {
                    if (GetComponent<MapCollisionState>(mapElement.value).value.TryGetValue(currentPoint, out Entity collidableEntity)) {
                        ...
                        if (currentPoint.ComparePoints(mapBody.point) && !currentPoint.ComparePoints(proj.lastPiercePoint)) {
                            DynamicBuffer<Reactics.Core.Effects.EffectBuffer> effects = effectBufferFromEntity[collidableEntity];
                            effects.Add(new Reactics.Core.Effects.EffectBuffer { effect = proj.effect });
                            proj.currentPierce++;
