...
public class DeleteProjectilesSystem : SystemBase {
    ...
    protected override void OnUpdate() {
        ...
        Entities.ForEach((int entityInQueryIndex, Entity entity, in Reactics.Komota.Projectile proj) =>
        {
...
public class SetTargetPosition : SystemBase {
    protected override void OnUpdate() {
        ...
        Entities.WithNone<MapBody>().ForEach((ref Reactics.Komota.Projectile proj) =>
        {
...
public class RotateTowardTargetSystem : SystemBase {
    protected override void OnUpdate() {
        Entities.ForEach((ref Rotation rot, in Translation trans, in Reactics.Komota.Projectile proj) =>
        {
            float3 direction = proj.targetPosition - trans.Value;
            rot.Value = quaternion.LookRotation(math.normalize(direction), Vector3.up);
        }).WithChangeFilter<Reactics.Komota.Projectile>().Schedule();
    }
}
...
public class ArcForwardSystem : SystemBase {
    protected override void OnUpdate() {
        ...
        Entities.WithNone<MapBody>().ForEach((ref Translation trans, ref Reactics.Komota.Projectile proj, in Rotation rot) =>
        {
...
public class DetectCollisionSystem : SystemBase {
    protected override void OnUpdate() {
        ...
        BufferFromEntity<Reactics.Komota.EffectBuffer> effectBufferFromEntity = GetBufferFromEntity<Reactics.Komota.EffectBuffer>(false);
        ...
        Entities.WithNone<MapBody>().ForEach((ref Translation trans, ref Reactics.Komota.Projectile proj, in MapElement mapElement) =>
        {
            ...
            if (!currentPoint.ComparePoints(proj.originPoint) && proj.currentPierce < proj.effect.maxPierce) {
                if (GetComponent<MapCollisionState>(mapElement.value).value.TryGetValue(currentPoint, out Entity collidableEntity)) {
                    ...
                    if (currentPoint.ComparePoints(mapBody.point) && !currentPoint.ComparePoints(proj.lastPiercePoint)) {
                        DynamicBuffer<Reactics.Komota.EffectBuffer> effects = effectBufferFromEntity[collidableEntity];
                        effects.Add(new Reactics.Komota.EffectBuffer { effect = proj.effect });
                        proj.currentPierce++;
