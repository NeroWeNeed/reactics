...
public class ActionIssueSystem : SystemBase {
    ...
    protected override void OnCreate() {
        ...
        archetype = EntityManager.CreateArchetype(new ComponentType[] {
            typeof(Reactics.Komota.Projectile),
            typeof(Translation),
            typeof(Rotation),
            typeof(RenderMesh),
            typeof(LocalToWorld),
    ...
    protected override void OnUpdate() {
        ...
        Entities.WithNone<FindingPathInfo/*,MapBodyTranslationStep*/>().ForEach((int entityInQueryIndex, Entity entity, ref Reactics.Komota.Projectile proj, ref ActionMeterData meter, in Translation translation, in UnitStatData unitData, in HealthPointData healthPointData, in MagicPointData magicPointData) =>
        {
            //Get rid of the component immediately.//what if we skip makingentities and go straight to applying the dynamic buffer?
            ecb.RemoveComponent<Reactics.Komota.Projectile>(entityInQueryIndex, entity);
            ...
            Reactics.Komota.Projectile newProj = proj;
