...
namespace Reactics.Core.Editor.Graph {
    public class ValuePortElement : VisualElement {
        ...
        private void Init(VisualElement element, Direction direction, Type portType) {
            ...
            port = PortUtility.Create(portType, null, direction, Port.Capacity.Single, Color.white);
            this.Add(port);
        ...
        private void OnDetachFromPanel(DetachFromPanelEvent evt) {
            if (attacher != null) {
                ...
                /*                 foreach (var con in port.connections) {
                                ...
                                foreach (var con in container.port.connections) {
                                    con.RemoveFromHierarchy();
                                }
                                port.DisconnectAll();
        ...
        public class Container : VisualElement {
            ...
            public Container(ValuePortElement source) {
                ...
                this.port = PortUtility.Create(source.port.portType, null, source.port.direction == Direction.Input ? Direction.Output : Direction.Input, Port.Capacity.Single, Color.white);
